...
- `local-doc-search codex-tui-search`: Codex session履歴の検索結果をfzfで選択し、session JSONLをpager/editorで開く。
...
`local-doc-search codex-tui-search` は `codex-search` と同じ固定DB・検索引数を使い、結果を `fzf` で選択できるようにする。

- `--db` と `--model` は指定しない。
- server discoveryとdirect fallbackは `codex-search` と同じ。
- 一覧表示では `score`, `source`, DB名, `role`, `session_id:line_no`, `cwd` を表示する。
- previewと選択後のopenでは、Codex履歴metadataの `session_path` と `line_no` を優先する。
- 既定では `less +<line_no> <session_path>` を起動する。
- `--pager`, `--preview-lines`, `--no-preview`, `--preview-window` は通常の `tui-search` と同じ。
